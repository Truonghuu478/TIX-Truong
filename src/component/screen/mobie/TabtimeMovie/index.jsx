...
function MovieMobile(props) {
  let { detailCinemaToTheater } = props;
  ...
  const filterTheaters = (theater) => {
    let RapImg = dataMaHeThongRap.find(
      (cumRap) => cumRap.maHeThongRap === theater.maHeThongRap
    );
    return theater.cumRapChieu.map((cumRap, index) => {
      ...
      return (
        <React.Fragment key={index}>
          {" "}
          <Typography className={classes.cumRapChieu} component="div">
            <img
              src={RapImg.hinhAnh}
              width={50}
              height={50}
              alt={"logo-" + RapImg.maHeThongRap}
            />
            <Typography style={{ paddingLeft: 10 }} component="div">
              ...
              <p style={{ color: "#9b9b9b", marginBottom: 0, fontSize: 14 }}>
                890 Trần Hưng Đạo, Q.5
              </p>
            </Typography>
          </Typography>
    ...
    const arrDayTAb = [...Array(14)].map((item, index) => {
      ...
      return (
        <Tab
          key={index}
          style={{ maxWidth: 72 }}
          className={classes.tabSpan}
          label={
            <>
              <span className={`d-block ${classes.span}`}>
                {arrDay[nextDay.getDay()]}
              </span>
  ...
  }, [filterTheaters, detailCinemaToTheater]);
