  ...
  const _renderHTML = React.useCallback( () => {
    ...
    if (arrTheater.length > 0) {
      ...
      return arrTheater[0].cumRapChieu.map((cinema, index) => {
        ...
        let arrRap = dataMaHeThongRap.filter(rap => rap.maHeThongRap == props.maHeThongRap)
              ...
              <Typography component={"div"} className={classes.typography}>
                ...
                <div  className={classes.cinema__info}>
                  ...
                  <span className={classes.cinema__name}  >
                    <span
                      style={{ color: `${clusterName.color}` }}>{clusterName.name} </span> - {lastChar[1] === " CGV Saigonres Nguyễn Xí"?lastChar[1].split(" ").map((char,index)=>index !== 1 ? char + " ":null):lastChar[1]}
                  </span>
