...
const useStyles =  makeStyles(()=>(
 ...
 function TimesMovie(props) {
    ...
    const {chonTheoNgayChieu} = useSelector(state => state.MovieManaGerment)
     let {cinema} = props;
    let screenWidth = useMemo(() =>window.innerWidth);
    ...
    const renderInfoMovie = () => {
        ...
        if (Object.entries(cinema).length >0) {
          let a= cinema.lichChieuPhim.filter(
            ...
            return a.length >0 ? a.map((lich,index) => {
              return (
                <Link 
                key={index}
                
                 to={props.userLogin  || localStorage.getItem("user") ?`/checkout/${lich.maLichChieu}`:"/login"} >
                  <Button
                    onClick={
                      () => {
                        let {maLichChieu} = lich;
                      dispatch({type:"CHECK_LOGIN_BOOKING",
                      scheduleCode:maLichChieu})  }}
                  style={{padding:screenWidth > 768 ?"8px" :"12px 6px",fontSize:screenWidth > 768 ? 14 : 17}}
                    ...
                    <span className="text-secondary">
                      {parseInt(lich.ngayChieuGioChieu.slice(11, 16)) +
                        1 <
                        10
                        ? `0${
                        parseInt(
            ...
            }):  <Alert severity="error">Xin lỗi Lịch chiếu chưa cập nhật, mời bạn chọn rạp khác !</Alert>
