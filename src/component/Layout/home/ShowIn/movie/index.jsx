...
function MovieShowIn(props) {
  ...
  const renderMovie = React.useCallback(() => {
    ...
    if ((indexHome !== 0 ? movieToDay : dataMovieFist).length > 0) {
      return (indexHome !== 0 ? movieToDay : dataMovieFist).map(
        (movie, index) => {
          ...
          return (
            <Accordion key={index} defaultExpanded>
              ...
              <AccordionDetails>
                <Typography>{_renderShowTimeMovie(lichChieu)}</Typography>
              </AccordionDetails>
            </Accordion>
          );
        }
  ...
  const _renderShowTimeMovie = React.useCallback(
    (lstSuatChieu) => {
      return lstSuatChieu.map((lc, index) => {
        ...
        return (
          <Link
            key={index}
            to={props.userLogin ? `/checkout/${lc.maLichChieu}` : "/login"}
          >
            <Button
              onClick={() => {
                props.__checkOutRePage(lc.maLichChieu);
              }}
              variant="contained"
