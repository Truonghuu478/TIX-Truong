...
// render sap chieu 
const renderSlideDangChieu = React.useCallback((arraySetups =dangChieuData )=>{
  ...
  return [...Array(3)].map((item,index)=>{
    ...
    return <SwiperSlide key={index } tag="div">
      
            <Grid container spacing={screenWidth > 768 ? 6 : 0}  >
            {arraySetups.map((movie,index1)=>{
              if(index === 1 ? index1 >8  && index1 < 17 : index === 2 ? index1 > 16 : index1 < 8  ){

      
              return <Grid key={index1} 
              style={{padding :screenWidth >768 ? null :20}}  item xs={12}  sm={6} md={4} lg={3}>
                       {screenWidth > 677 ?<Card   
                       className={`${arraySetups === dangChieuData ? classes.cardDangChieu :classes.cardSapChieu}  ${classes.Card}`} >
                          <CardActionArea variant="contained"  >
                            ...
                            />
                            {screenWidth >= 768 && <CardContent className={classes.CardContent}  >
                              <Box  className={classes.boxMovie} component="div"
                                item={"true"}>
                                  <Typography component="span" 
                                  className={`${classes.banner} ${movie.room=== 16 ?classes.banner16:classes.bannerP }`}>
                                      {movie.room=== 16 ? 16:"P" }
                                  </Typography>
                                    {movie.tenPhim} - {movie.room=== 16 ? "(16)":"(P)" }
                              </Box>
                          ...
                          {arraySetups === dangChieuData && <Box className={classes.checkStart} component="div">
                            <Typography
                             style={{fontWeight:"bold",color:"white",textAlign:"center",marginBottom:0,transform:"scaleY(1.2)"}}
                              component="p">
                              {movie.danhGia}
                            </Typography>
                            <Typography  component="span">
                            ...
                            {/* hover  */}
                          < Box style={{display: screenWidth <768? "none":"block"}} className={classes.boxMovieOver} component="div">
                              <Box className={classes.boxMovieOverImg} 
                              onClick={()=>handleGetTrailer(movie.trailer)} component="div">

                                <CardMedia  component="img"
                                 image={"https://tix.vn/app/assets/img/icons/play-video.png"} />
                              </Box>

                                
                            {arraySetups === dangChieuData && <CardActions
                            onClick={()=>{handleCheckLogin(movie.maPhim,movie.room)}} className={classes.boxMovieOverBtn} > 
                              <Link style={{width:"100%"}} to={`/phim/${movie.maPhim}`}>
                                <Button className={classes.btnMovie} size="small" >
                                  Mua vé
                                </Button>
                              </Link>
                          ...
                          {/* end hover  */}
                          <CardMedia className={classes.datTruoc} component="img"
                           
                             image={"https://tix.vn/app/assets/img/icons/film_type_3.png"} />
                        </Card>:
                        
                        <Link
                        onClick={()=>{handleCheckLogin(movie.maPhim,movie.room)}}
                         style={{width:"100%"}} to={`/phim/${movie.maPhim}`}>
  ...
  return (
    ...
    <ModalTrailer 
    onHide={()=>{setModalTrailer(false)}} 
    show={modalTrailer} trailer={trailer} />
