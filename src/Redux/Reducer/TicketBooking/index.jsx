...
const BookingReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case tyAction.GET_COUNT_CHAIR: {
          ...
          if(action.name === "Vé thường" && (state.sttNormal >= 0 || state.sttNormal <= 11)){
          ...
          }else if(action.name === "Vé vip" &&(state.sttVip >= 0 || state.sttVip <= 11)){ 
            if (action.result ) {
    ...
    let newArrChairs = [...state.listChairs];
      ...
      let delNormal =  newArrChairs.findIndex(item=>item.loaiGhe === "Thuong")
 ...
 if(data.daDat){
   if(newArrChairs.length>0 && (sttNormal !== 1 || sttVip !== 1)){
       ...
       for(let i=newArrChairs.length -1;i>=0;i--){
         ...
         if(parseInt(newArrChairs[i].stt) + 1 === parseInt(data.stt) 
         ...
         break;
       }else if(data.loaiGhe ==="Thuong" && newArrChairs[i].loaiGhe === "Thuong" ) {
        if(parseInt(newArrChairs[i].stt) + 1 === parseInt(data.stt) 
