  ...
  switch (action.type) {
    case tyAction.POST_VALUE_SEARCH: {
      ...
      switch (statusAdmin) {
        case "/admin/user": {
          ...
          if (payload) {
            const postArray =
              listUsers.length > 0 ? listUsers : defaultListUser;

            newRows = postArray.filter(
              (item) =>
                item.taiKhoan.toLowerCase().indexOf(payload.toLowerCase()) >
                  -1 ||
                item.hoTen.toLowerCase().indexOf(payload.toLowerCase()) > -1
            );
          }
          // let ;
        ...
        case "/admin/movie-theater": {
          ...
          if (payload) {
            const postMovie =
              listMovies.length > 0 ? listMovies : defaultListMovie;
            newMovie = postMovie.filter(
              (movie) =>
                movie.tenPhim.toLowerCase().indexOf(payload.toLowerCase()) >
                  -1 ||
                movie.moTa.toLowerCase().indexOf(payload.toLowerCase()) > -1
            );
          }
    ...
    case tyAction.GET_LIST_USERS: {
      return {
        ...
        defaultListUser: action.payload,
        globalSearch: "",
      };
    }
    ...
    case tyAction.CHANGE_STATUS_INDEX: {
      return { ...state, indexSpinner: action.statusIndex, globalSearch: "" };
    }
    ...
    case tyAction.CHANGE_STATUS_MOVIES: {
      return {
        ...state,
        listMovies: action.payload,
        defaultListMovie: action.payload,
      };
    }
    default:
      return { ...state };
